...
# Добавьте асинхронный генератор get_user_db. Он обеспечивает доступ к БД
# через SQLAlchemy и в дальнейшем будет использоваться в качестве
# зависимости (dependency) для объекта класса UserManager:
async def get_user_db(session: AsyncSession = Depends(get_async_session)):
    yield SQLAlchemyUserDatabase(session, User)


# Добавьте компоненты, необходимые для построения аутентификационного бэкенда
# — транспорт, стратегию и собственно сам объект бэкенда.
# В FastAPI Users можно создать и использовать одновременно несколько бэкендов,
# поэтому у каждого из них должно быть своё уникальное имя.
...
# Добавьте класс UserManager и корутину, возвращающую объект этого класса.
# Класс UserManager должен быть унаследован: IntegerIDMixin BaseUserManager
